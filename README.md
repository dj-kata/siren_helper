...
(出現モンスターについては現状、通常神髄及び超・神髄のみ対応)

価格識別については店内でアイテムの上に乗った場合のみ読み取ります。  
アイテム欄から識別済みのアイテムを自動で反映することもできます。
...
メモ欄は冒険用と全体用の2つを用意しており、前者はリセット(=ゲームオーバー)時に消えるようにしています。
...
# 設定方法
## インストールおよび実行
1. [リリースページ](https://github.com/dj-kata/siren_helper/releases)から最新のsiren6_helper.zipをダウンロードし、好きなフォルダに解凍する
...
文字認識を定期的に実行するため、PCへの負荷がやや大きくなります。価格識別が特に重いので、負荷状況と相談して無効にする/店にいるときのみonにするなど使い方を決めるとよいかと思います。

Switch版の場合は[こちら](https://github.com/dj-kata/siren_helper/wiki/OBS%E9%80%A3%E6%90%BA%E6%A9%9F%E8%83%BD%E5%88%A9%E7%94%A8%E6%96%B9%E6%B3%95)を参考にOBS WebSocket設定を行ってください。(かなり重いです)
...
`リセット`を押すことで、アイテム識別、現在の階層、メモ(1冒険用)をすべて初期化します。
...
買値/売値を選択するとさらに絞り込むことができます(多くの場合)。  
